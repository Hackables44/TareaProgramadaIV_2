...
FabricaVector::FabricaVector(const char * tipoProducto, const char * nombreProducto):Fabrica(tipoProducto, nombreProducto){
	/** indico que el objeto que se utiliza es de tipo string, es como hacer setTipoProducto("string");  */ 
	/** Asigno como nombre del producto, el nombre de la clase de la cual produce objetos, es como hacer setNombreProducto("Vector"); */
}
...
/** - Invoca al constructor de la clase Vector y devuelve el puntero que recibe */
Producto * FabricaVector::producir(){
	return new Vector(); /** invoca al constructor de la clase Vector y devuelve el puntero que recibe */
}